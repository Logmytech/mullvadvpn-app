...
impl DnsMonitor {
    ...
    fn write_state(&self) -> Result<()> {
        ...
        if let Some(ref nameservers) = self.state {
            let valid_nameservers = nameservers
                .iter()
                .filter_map(
                    |nameserver_string| match nameserver_string.parse::<IpAddr>() {
            ...
            config.set_nameservers(valid_nameservers);
        } else {
            config.set_nameservers::<IpAddr>(vec![]);
        }
